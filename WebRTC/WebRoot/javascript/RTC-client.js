var WebRTC = function() {
    ...
    webrtc.prototype.connect = function(server, room) {
        ...
        this.on('_remove_peer', function(data) {
            var sendId;
            that.closePeerConnection(that.peerConnections[data]);
            delete that.peerConnections[data];
            delete that.dataChannels[data];
            for (sendId in that.fileChannels[data]) {
                that.emit("send_file_error", new Error("Connection has been closed"), data, sendId, that.fileChannels[data][sendId].file);
            }
            delete that.fileChannels[data];
            that.emit("remove_peer", data);
        });
            ...
            pcCreateOfferCbGen = function(pc, socketId) {
                return function(session_desc) {
                    ...
                    that.socket.send(JSON.stringify({
                        "eventName": "__offer",
                        "data": JSON.stringify({
                            "sdp": JSON.stringify(session_desc),
                            "socketId": socketId
                        })
                    }));
                };
    ...
    webrtc.prototype.createPeerConnections = function() {
        var i, m;
		//console.log("hh"+this.connections.length)
        for (i = 0, m = this.connections.length; i < m; i++) {
    ...
    webrtc.prototype.sendFile = function(dom, socketId) {
        ...
        file = dom.files[0];//file类型的input获取所选择的文件
    ...
    webrtc.prototype.receiveFileAccept = function(sendId, socketId) {
        var that = this,
            ...
            initSending = function(event, text) {
                ...
                fileToSend.fileData = event.target.result;//这是reader读到的文件数据
